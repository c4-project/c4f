...
| Location_global v1,Location_global v2 ->
    Parsed_constant.compare v1 v2
| Location_deref (v1,i1),Location_deref (v2,i2) ->
    begin match Parsed_constant.compare v1 v2 with
...
let dump_location = function
  ...
  | Location_global v -> Parsed_constant.pp_v v
  | Location_deref (v,i) ->
      Printf.sprintf "%s[%i]" (Parsed_constant.pp_v v) i

let dump_rval loc = match loc with
  | Location_reg (i,r) -> Printf.sprintf "%i:%s" i r
  | Location_sreg s -> Misc.dump_symbolic s
  | Location_global v -> Printf.sprintf "*%s" (Parsed_constant.pp_v v)
...
type prop = (location, maybev) Constr_gen.prop
type constr = prop Constr_gen.constr
type quantifier = Constr_gen.kind
...
type extra_data =
  | NoExtra
  | CExtra of Ast.param list list
  | BellExtra of Bell_info.test
...
type ('i, 'p, 'prop, 'loc) result =
    { info : info ;
      ...
      prog : 'p ;
      filter : 'prop option ;
      condition : 'prop Constr_gen.constr ;
