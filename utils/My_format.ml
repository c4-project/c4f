...
let pp_listlist ~pp =
  Fmt.(vbox (list ~sep:cut (hvbox (brackets (list ~sep:comma pp)))))

let pp_c_braces (pi : 'a Fmt.t) : 'a Fmt.t =
  Fmt.(
    hvbox
      (suffix (unit "@ }") (hvbox ~indent:4 (prefix (unit "{@ ") pi)))
  )
;;
...
let pp_kv f k pv v =
  Fmt.(hvbox ~indent:1
         (pair ~sep:(const (suffix sp char) ':') string pv))
    f (k, v)
;;
