    ...
    let%expect_test "sample" =
      ...
      [%expect
        {|
          ...
          (atomic_load_explicit(bar, memory_order_seq_cst) == 95 && barbaz || false ||
           barbaz || true && true && true &&
           atomic_load_explicit(&y, memory_order_seq_cst) == 53 &&
           atomic_load_explicit(bar, memory_order_consume) == -9790791 || barbaz ||
           atomic_load_explicit(&x, memory_order_consume) ==
           atomic_load_explicit(&y, memory_order_relaxed) || -1 == 13585 || false ||
           foo == 4 && true || 84 == -245825 || barbaz)
          (atomic_load_explicit(&x, memory_order_seq_cst) == 27 || true &&
           atomic_load_explicit(&z, memory_order_seq_cst) == false || foo == 1234853 ||
           atomic_load_explicit(&x, memory_order_relaxed) == *blep || false)
          (*blep == 99 && *blep == 99 || barbaz && true)
          (atomic_load_explicit(bar, memory_order_seq_cst) == 95 &&
           atomic_load_explicit(bar, memory_order_seq_cst) == 95 || false && true)
          (false || true)
          (false || false || atomic_load_explicit(&z, memory_order_seq_cst) == false &&
           true)
          (true || true)
          (true || barbaz)
          ...
          (barbaz || atomic_load_explicit(foobaz, memory_order_seq_cst) == true &&
           atomic_load_explicit(&z, memory_order_seq_cst) == false)
          (-5 == *blep || -312 == *blep || 1317973 == foo || true ||
