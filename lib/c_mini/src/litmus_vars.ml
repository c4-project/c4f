...
let lift_global_var_alist (type a)
  (xs : (Ac.C_id.t, Type.t) List.Assoc.t)
...
let make_global_var_alist (type a)
  (progs : Litmus.Ast.Lang.Program.t list)
  ~(f:Ac.C_id.t -> Type.t -> a)
  : (Ac.Litmus_id.t, a) List.Assoc.t Or_error.t
  = match progs with
    | [] ->
    ...
    | x :: xs ->
      ...
      Or_error.(
        xs
        |> List.map ~f:(Fn.compose (check_parameters_consistent params) Named.value)
        |> Or_error.combine_errors_unit
        >>= fun () -> lift_global_var_alist ~f params)

let make_local_var_alist (type a)
    (tid : int)
