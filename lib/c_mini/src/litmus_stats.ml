...
let fold_fn (stats : t) (fn : unit Function.t Act_common.C_named.t) : t =
  fn |> Act_common.C_named.value |> Function.body_stms
  |> List.fold ~init:stats ~f:fold_top_stm

let scrape (t : Litmus.Test.t) : t =
  let ts = Litmus.Test.threads t in
  let threads = List.length ts in
  List.fold ts ~init:{threads; returns= 0; literal_bools= 0} ~f:fold_fn
