...
let fence_signal_name : string = "atomic_signal_fence"
...
let model_cmpxchg_with_args ~(raw_obj : Ast.Expr.t)
  ...
  Or_error.Let_syntax.(
    ...
    and fail = Convert_prim.expr_to_memory_order raw_fail (* memory_order *) in
    Atomic_cmpxchg.make ~obj ~expected ~desired ~succ ~fail
  )

let model_cmpxchg (args : Ast.Expr.t list) ~(expr : Ast.Expr.t -> Expression.t Or_error.t) : Expression.t Atomic_cmpxchg.t Or_error.t =
  match args with
  | [raw_obj; raw_expected; raw_desired; raw_succ; raw_fail] ->
      model_cmpxchg_with_args ~raw_obj ~raw_expected ~raw_desired
...
let model_fetch_with_args ~(raw_obj : Ast.Expr.t)
    ~(raw_arg : Ast.Expr.t)
    ~(raw_mo : Ast.Expr.t)
    ~(op : Op.Fetch.t)
  ~(expr : Ast.Expr.t -> Expression.t Or_error.t):
  ...
  Or_error.Let_syntax.(
    ...
    and mo = Convert_prim.expr_to_memory_order raw_mo (* memory_order *)
    in
    Atomic_fetch.make ~obj ~arg ~mo ~op
  )

let model_fetch (args : Ast.Expr.t list) ~(op: Op.Fetch.t) ~(expr : Ast.Expr.t -> Expression.t Or_error.t) : Expression.t Atomic_fetch.t Or_error.t =
  match args with
  ...
  | _ ->
      Or_error.error_s
        [%message
          "Invalid arguments to atomic fetch"
            ~got:(args : Ast.Expr.t list)]

let model_fence (args : Ast.Expr.t list) ~(mode : Atomic_fence.Mode.t) :
    Atomic_fence.t Or_error.t = match args with
  | [raw_mo] ->
      Or_error.Let_syntax.(
...
let model_load : Ast.Expr.t list -> Atomic_load.t Or_error.t =
  function
  | [raw_src; raw_mo] ->
      Or_error.Let_syntax.(
...
let model_store (args : Ast.Expr.t list) ~(expr : Ast.Expr.t -> Expression.t Or_error.t): Atomic_store.t Or_error.t =
  match args with
