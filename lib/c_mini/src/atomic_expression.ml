...
let reduce (ae : 'e t) ~(cmpxchg : 'e Atomic_cmpxchg.t -> 'a)
    ~(fetch : 'e Atomic_fetch.t -> 'a) ~(load : Atomic_load.t -> 'a)
    ~(xchg : 'e Atomic_xchg.t -> 'a) : 'a =
  match ae with
  | Cmpxchg c ->
      cmpxchg c
  | Fetch f ->
...
module Base_map (Ap : Applicative.S) = struct
  let bmap (x : 'a t)
      ...
      ~(xchg : 'a Atomic_xchg.t -> 'b Atomic_xchg.t Ap.t) : 'b t Ap.t =
...
Travesty.Traversable.Make1 (struct
  ...
  module On_monad (M : Monad.S) = struct
    ...
    module Ac = Atomic_cmpxchg.On_expressions.On_monad (M)
    module Af = Atomic_fetch.On_expressions.On_monad (M)
    module Ax = Atomic_xchg.On_expressions.On_monad (M)
...
module Type_check (E : Env_types.S) :
  ...
  module Ax = Atomic_xchg.Type_check (E)
