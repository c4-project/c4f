...
module Str_result : Monad.S with type 'a t = ('a, string) Result.t =
  struct
...
module Make_alist
  ...
  let to_yojson (x : t) : Yojson.Safe.t =
    `Assoc
      (Tx.Alist.bi_map ~left:K.to_string ~right:V.to_yojson x)

  let protect_key_of_string (k : string) : K.t Str_result.t =
    protect ~f:(fun () -> K.of_string k)

  let require_assoc (j : Yojson.Safe.t) : (string, Yojson.Safe.t) List.Assoc.t Str_result.t =
    protect ~f:(fun () -> Yojson.Safe.Util.to_assoc j)

  let of_yojson : Yojson.Safe.t -> t Str_result.t =
    Sx.(require_assoc >=> AM.bi_map_m ~left:protect_key_of_string ~right:V.of_yojson)
end
...
module Make_map
    ...
    (V : Jsonable_types.S)
  : Jsonable_types.S with type t = V.t Map.M(K).t = struct
...
module Option (B : Jsonable_types.S) : Jsonable_types.S with type t = B.t option = struct
  type t = B.t option
