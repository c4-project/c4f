  ...
  struct
    ...
    let address_globals :
      ...
      C.Mini.Statement.On_addresses.With_errors.map_m
        ~f:
          (Ctx.T.when_global ~over:C.Mini.Address.variable_of
             ~f:(fun addr ->
        ...
        ~f:
          C.Mini.Lvalue.(
            Ctx.T.when_global ~over:variable_of
              ~f:
                (Fn.compose Or_error.return
                   (Fn.compose variable variable_of)))
    ...
    let rewrite_statement (stm : C.Mini.Statement.t) :
