...
let parameter_list_equal :
   C.Mini.Type.t C.Mini_intf.id_assoc ->C.Mini.Type.t C.Mini_intf.id_assoc -> bool
    =
  [%equal: (Ac.C_id.t * C.Mini.Type.t) list]

let check_parameters_consistent (params :C.Mini.Type.t C.Mini_intf.id_assoc)
...
struct
  ...
  let ref_globals :C.Mini.Statement.t ->C.Mini.Statement.t =
   C.Mini.Statement.On_lvalues.map
      ~f:
       C.Mini.Lvalue.(
          T.when_global ~over:variable_of ~f:(Fn.compose variable variable_of))

  (** [proc_stm stm] runs all of the global-handling functions on a single
      statement. *)
  let proc_stm (stm :C.Mini.Statement.t) :C.Mini.Statement.t =
    stm |> address_globals |> ref_globals |> Qualify.locals_in_statement (module T)
...
let delitmus_function (tid : int) (func :C.Mini.Function.t) :C.Mini.Function.t
    =
  let locals =C.Mini.Function.body_decls func in
 C.Mini.Function.map func ~parameters:(Fn.const []) ~body_decls:(Fn.const [])
    ~body_stms:(delitmus_stms tid locals)

let delitmus_functions :
   C.Mini.Function.t C.Mini_intf.id_assoc ->C.Mini.Function.t C.Mini_intf.id_assoc
    =
