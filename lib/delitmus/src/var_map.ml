...
let lookup_and_require_global (map : t) ~(id : Ac.Litmus_id.t)
  ...
  | None ->
    Or_error.error_s
      [%message "Litmus identifier doesn't match any in the auxiliary var map"
          ~id:(id : Ac.Litmus_id.t)]
  | Some None ->
    Or_error.error_s
      [%message "Litmus identifier was mapped to something other than a global variable"
          ~id:(id : Ac.Litmus_id.t)]
  | Some (Some x) -> Or_error.return x
