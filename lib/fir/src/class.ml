...
struct
  ...
  let satisfies (type e) (elt : e elt) ~(req : Class_constraint.t)
    ...
    match req with
    | Is -> matches_any elt ~templates
    | Is_not_any -> not (matches_any elt ~templates)
    | Is_not_one -> unmatches_any elt ~templates
    | Has -> rec_matches_any elt ~templates
    | Has_not_any -> not (rec_matches_any elt ~templates)
    | Has_not_one -> rec_unmatches_any elt ~templates
end
