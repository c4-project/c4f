...
module Int_zeroes (E : Env_types.S) = struct
  ...
  let gen_kv_refl_op : (t -> t -> t) Q.Generator.t =
    Q.Generator.Let_syntax.(
      let%bind op = gen_refl_zero_op in
      let op = Expression.bop op in
      let%map flip = Q.Generator.bool in
      fun l r ->
        (* Even though the operators are likely not commutative, we're
  ...
  let recursive_generators (mu : t Q.Generator.t) : t Q.Generator.t list =
    eval_guards
      ...
      ; (Det_prims.has_ints_any_atomicity (), fun () -> gen_kv_refl ~mu) ]
...
module Atomic_fetch_int_refl_nops (Obj : Env_types.S) :
  ...
  let gen_op :
      (Address.t -> Expression.t -> Expression.t Atomic_fetch.t)
      Q.Generator.t =
    Q.Generator.Let_syntax.(
