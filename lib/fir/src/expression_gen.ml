...
module Int_prims (E : Env_types.S) = struct
  ...
  let lift (g : 'a Q.Generator.t) ~(to_expr : 'a -> t)
      ...
      =
    Q.Generator.map ~f:(fun (l, r) -> (to_expr l, r)) (with_record g ~to_var)
  ...
  let gen_atomic_fetch_zero_nop ~(gen_zero : t Q.Generator.t) :
    ...
    let module F =
      Atomic_fetch.Quickcheck_ints (E) (Op.Fetch.Gen_idem_zero_rhs)
        (struct
          type nonrec t = t
  ...
  let gen_kv_refl (type v a) ~(gen_op : v -> t -> a Q.Generator.t)
      ~(gen_load : (v * Env.Record.t) Q.Generator.t) : a Q.Generator.t =
    Q.Generator.Let_syntax.(
      let%bind v, kv =
        Q.Generator.filter_map gen_load ~f:(fun (l, r) ->
            Option.Let_syntax.(
              let%map kv = Accessor.get_option Env.Record.known_value r in
              (l, Expression.constant kv)))
      in
      gen_op v kv)
...
module Int_zeroes (E : Env_types.S) = struct
  ...
  let recursive_generators (mu : t Q.Generator.t) : t Q.Generator.t list =
    eval_guards
      ...
      ; (Det_prims.has_ints_any_atomicity (), fun () -> gen_kv_zero_bop ~mu)
      ]
...
module Atomic_fetch_int_zero_nops (Obj : Env_types.S) (Arg : Env_types.S) :
  Act_utils.My_quickcheck.S_with_sexp
    with type t = Expression.t Atomic_fetch.t =
  Atomic_fetch.Quickcheck_ints (Obj) (Op.Fetch.Gen_idem_zero_rhs)
    (Int_zeroes (Arg))

module Atomic_fetch_int_refl_nops (Obj : Env_types.S) :
  Act_utils.My_quickcheck.S_with_sexp
    with type t = Expression.t Atomic_fetch.t = struct
  type t = Expression.t Atomic_fetch.t [@@deriving sexp]

  (* TODO(@MattWindsor91): a LOT of this overlaps with the atomic fetch
     generator... *)

  module A = Address_gen.Atomic_int_pointers (Obj)
  module P = Int_prims (Obj)
  module Z = Int_zeroes (Obj)

  let quickcheck_observer =
    [%quickcheck.observer: Expression.t Atomic_fetch.t]

  (* TODO(@MattWindsor91): expression shrinker *)
  let quickcheck_shrinker =
    [%quickcheck.shrinker: [%custom Q.Shrinker.atomic] Atomic_fetch.t]

  let gen_address : (Address.t * Env.Record.t) Q.Generator.t =
    P.with_record A.quickcheck_generator ~to_var:Address.variable_of

  let gen_op (obj : Address.t) (arg : Expression.t) :
      Expression.t Atomic_fetch.t Q.Generator.t =
    Q.Generator.Let_syntax.(
