...
let run (_o : Act_common.Output.t) (cfg : Act_config.Global.t) : unit Or_error.t =
...
let command : Core_kernel.Command.t =
  Command.basic ~summary:"dumps the current configuration to stdout"
    ~readme
    Command.Let_syntax.(
      let%map standard_args = Common_cmd.Args.Standard.get in
      fun () ->
        Common_cmd.Common.lift_command standard_args ~f:run)
