...
let%test_module "run" = (module struct
  ...
  let location_map : Ac.Litmus_id.t option Map.M(Ac.Litmus_id).t =
    ...
    |> Tx.Alist.bi_map ~left:Ac.Litmus_id.of_string ~right:(Option.map ~f:Ac.Litmus_id.of_string)
  ...
  let test (subject: Ob.t) : unit =
    let result = Src.Diff.run ~oracle:normal_oracle ~subject ~location_map in
