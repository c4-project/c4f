...
val make :
  ...
  -> ?flags:Act_fuzz.Flag.t Map.M(Act_common.Id).t
  -> unit
  -> t
...
val flags : t -> Act_fuzz.Flag.t Map.M(Act_common.Id).t
(** [flags cfg] gets the user-provided flag parameter settings in [cfg]. *)

(** {3 Making a parameter map} *)

val make_param_map : t -> Act_fuzz.Param_map.t
...
val make_pool : t -> Act_fuzz.Action.Pool.t Or_error.t
...
val summarise :
  t -> Act_fuzz.Action.Summary.t Map.M(Act_common.Id).t Or_error.t
