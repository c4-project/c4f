...
module Make_surround (Basic : sig
  ...
  (** [path_filter] is the path filter used on paths to statement spans being
      surrounded. *)
end) : F.Action_types.S with type Payload.t = F.Path.Test.t =
...
module Atomic_surround :
  F.Action_types.S with type Payload.t = F.Path.Test.t =
...
module Sync_surround : F.Action_types.S with type Payload.t = F.Path.Test.t =
Make_surround (struct
  let subname = "sync"
