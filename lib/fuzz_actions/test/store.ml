...
let%test_module "store.make.int.normal" =
  ( module struct
    ...
    let random_state (store : Fir.Atomic_store.t Lazy.t) :
        Src.Store.Int.Payload.t Lazy.t =
      Lazy.Let_syntax.(
...
let%test_module "store.make.int.dead" =
  ( module struct
    ...
    let random_state : Src.Store.Int.Payload.t Lazy.t =
      Lazy.Let_syntax.(
...
let%test_module "xchgify" =
  ( module struct
    let test_action (payload : F.Path.Test.t) :
        F.Subject.Test.t F.State.Monad.t =
      Src.Store.Xchgify.run (Lazy.force FT.Subject.Test_data.test) ~payload

    let test (lpath : F.Path.Test.t Lazy.t) : unit =
    ...
    let%expect_test "example store" =
      test
        F.Path.(
          FT.Subject.Test_data.Path.thread_0_stms @@ Stms.in_stm 0
          @@ Stm.this_stm) ;
      [%expect
        {|
      void
