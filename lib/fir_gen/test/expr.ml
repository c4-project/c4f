...
let test_has_type (type_of : Fir.Expression.t -> Fir.Type.t Or_error.t)
  (e : Fir.Expression.t) (ty : Fir.Type.t) ~(here : Lexing.position)
  : unit =
      [%test_result: Fir.Type.t Or_error.t] (type_of e) ~here:[here; [%here]]
        ~equal:[%compare.equal: Fir.Type.t Or_error.t]
        ~expect:(Ok ty)

let test_in_env (env : Fir.Env.t) (e : Fir.Expression.t) ~(here : Lexing.position) : unit =
...
let test_evaluates_to (heap : Fir.Heap.t) (e : Fir.Expression.t) (k : Fir.Constant.t) ~(here : Lexing.position) : unit =
...
let test_all_expressions
  ?(evaluate_to : Fir.Constant.t option)
    (f : Fir.Env.t -> (module Q.Test.S with type t = Fir.Expression.t))
  ~(have_type : Fir.Type.t)
  ...
  ( module struct
    ...
    let%expect_test "sample" =
      ...
      [%expect
        {|
      0
      atomic_fetch_add_explicit(&y, barbaz != true ?
                                (0 |
      ...
      atomic_load_explicit(bar, memory_order_relaxed) | (0 ^ 0) & -1
      0 ^ 0
    ...
    let%expect_test "sample (environment has only atomic_int*)" =
      ...
      [%expect
        ...
        & (atomic_load_explicit(bar, memory_order_consume) ^ (0 ^ (false ? 0 : 0)))
        0 ^ 0
        atomic_load_explicit(bar, memory_order_consume) -
        atomic_load_explicit(bar, memory_order_consume) ^
    ...
    let%test_unit "all expressions have 'int' type and vars are in environment" =
    ...
    let%expect_test "sample" =
      ...
      [%expect
        {|
          ...
          0 | 0
          true ? 0 : *blep
          foo == 4 ? true ? -1 & 0 : atomic_load_explicit(&y, memory_order_acquire) :
          foo - 4 & (0 | atomic_fetch_add_explicit(&y, 0, memory_order_acquire))
          false &&
          (atomic_load_explicit(&z, memory_order_consume) ||
           atomic_load_explicit(foobaz, memory_order_relaxed))
          ? 0 ^ (-52859389 ^ -52859389) : 99 & 0 |}]
...
let%test_module "Atomic int nops" =
  ( module struct
    ...
    let%expect_test "sample" =
      ...
      [%expect
        {|
          atomic_fetch_sub_explicit(bar, false ? foo : true ? 0 :
                                    atomic_fetch_or_explicit(&x, 0,
                                                             memory_order_acquire),
          ...
          atomic_fetch_or_explicit(&y, foo & 0, memory_order_release)
...
let%test_module "Bool_values" =
  ( module struct
    ...
    let%expect_test "sample" =
      ...
      (atomic_load_explicit(&y, memory_order_acquire) ^ foo)
      !barbaz ||
      !((atomic_load_explicit(&y, memory_order_consume) | 0) <
        atomic_fetch_add_explicit(&x, 0 ^ 0, memory_order_seq_cst))
      !((atomic_fetch_add_explicit(bar, 0, memory_order_acq_rel) <= foo || false)
        &&
    ...
    let%expect_test "sample (environment has only atomic_int*)" =
      ...
      [%expect
        ...
        atomic_load_explicit(foobaz, memory_order_seq_cst)
        126030 != (0 ^ 0)
        atomic_fetch_sub_explicit(bar, false ?
                                  atomic_load_explicit(bar, memory_order_consume) :
                                  0, memory_order_release)
        >= (atomic_load_explicit(bar, memory_order_seq_cst) | 0)
        true && atomic_load_explicit(foobaz, memory_order_consume) ||
        (0 <= (atomic_fetch_add_explicit(bar, 0, memory_order_relaxed) ^ 0) ||
         (atomic_load_explicit(foobaz, memory_order_relaxed) &&
          atomic_load_explicit(foobaz, memory_order_relaxed) ||
    ...
    let%expect_test "sample (environment is empty)" =
      ...
      [%expect
        ...
        false
        true
        (-8191 & 18659626 ^ -1073741824 - -1073741824) < 0 - 0
        (-378 | 0) >= -879720314 || (-1 == -2 && 7627 <= 0 || (0 > 0 || true)) ||
        false
        !(0 != 2147483647) |}]

    let%test_unit "all expressions have 'bool' type and vars are in environment" =
      test_all_expressions
        (fun e ->
          ( module Src.Expr.Bool_values (struct
            let env = e
...
let%test_unit "all expressions evaluate to false and vars are in environment" =
      test_all_expressions
        (fun e ->
          let module B = (Src.Expr.Bool_known (struct
            let env = e
          end))
          in (module B.Falsehoods)
          )
        ~have_type:Fir.Type.(bool ())
        ~evaluate_to:(Bool false)
        ~here:[%here]
  end )
...
let%test_unit "all expressions evaluate to true and vars are in environment" =
      test_all_expressions
        (fun e ->
          let module B = (Src.Expr.Bool_known (struct
            let env = e
          end))
          in (module B.Tautologies)
          )
        ~have_type:Fir.Type.(bool ())
        ~evaluate_to:(Bool true)
        ~here:[%here]
