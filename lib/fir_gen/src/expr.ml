...
let gen_atomic_fetch_zero_nop (env : Fir.Env.t) ~(gen_zero : t Q.Generator.t)
...
let gen_int_loadlike (env : Fir.Env.t) ~(gen_zero : t Q.Generator.t) :
    (t * Fir.Env.Record.t) Q.Generator.t =
...
let gen_int_with_fetch_nops (env : Fir.Env.t) ~(gen_zero : t Q.Generator.t) :
    t Q.Generator.t =
  (* TODO(@MattWindsor91): &-1 fetches *)
  Q.Generator.weighted_union
    (Utils.My_list.eval_guards
...
module Int_zeroes (E : Fir.Env_types.S) = struct
  ...
  let kv_env = Fir.Env.filter_to_known_values E.env
  ...
  let gen_prim_zero_bop ~(mu : t Q.Generator.t) : t Q.Generator.t =
    (* This is [kv_env] because it depends on *)
    Q.Generator.Let_syntax.(
      let%bind p = gen_int_with_fetch_nops kv_env ~gen_zero:mu in
  ...
  let gen_kv_zero_bop ~(mu : t Q.Generator.t) : t Q.Generator.t =
    let gen_load = gen_int_loadlike kv_env ~gen_zero:mu in
    gen_kv_refl ~gen_load ~gen_op:(fun l r -> gen_zero_bop_any (Two (l, r)))

  let recursive_generators (mu : t Q.Generator.t) : t Q.Generator.t list =
    Utils.My_list.eval_guards
      [ (true, fun () -> gen_prim_zero_bop ~mu)
      ; ( Fir.Env.has_vars_of_prim_type kv_env ~prim:Int
...
module Atomic_fetch_int_refl_nops (Obj : Fir.Env_types.S) :
  ...
  let quickcheck_generator = gen_kv_refl ~gen_load:gen_address ~gen_op
end
