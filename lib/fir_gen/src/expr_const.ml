...
let ternary ~(truth : t Q.Generator.t) ~(arb : t Q.Generator.t) ~(mu : t Q.Generator.t) : t Q.Generator.t =
  Q.Generator.map ~f:Fir.Expression.ternary
    (Fir.Expr_ternary.quickcheck_generator_ite ~gen_if:truth ~gen_then:mu ~gen_else:arb)
...
let recursive_generators
  ...
  @
  List.filter_opt
    [ Some (2.0, ternary ~truth ~arb:k_gctx.arb ~mu)
...
let gen (k : Fir.Constant.t) (env : env) ~(int : env -> t Q.Generator.t)
  ...
  let rec mu (k : Fir.Constant.t) = (
    ...
    Q.Generator.weighted_recursive_union (base_generators k)
        ~f:(recursive_generators ~int_gctx ~bool_gctx ~k_gctx ?truth)
  )
  in mu k
