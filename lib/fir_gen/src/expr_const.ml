...
(** [ibop t] tries to make a generator for binary operations that take
...
let half (x : t Q.Generator.t) : t Q.Generator.t =
  Q.Generator.(
    Let_syntax.(
...
let arb_bop ~(gen_arb : t Q.Generator.t)
    ...
    Q.Generator.Let_syntax.(let%bind p = half gen_arb in bop (One p))
...
let kbop_generators ({kbop; arb; load; _} : gctx) : (float * t Q.Generator.t) list =
  List.filter_opt
    [ Option.map kbop ~f:(fun bop ->
              (3.0, arb_bop ~gen_arb:arb ~bop))
    ; Option.map2 kbop load ~f:(fun bop gen_load ->
                (5.0, kv_bop ~gen_load ~bop))
    ]

let ternary ~(gen_if : t Q.Generator.t) ~(gen_then : t Q.Generator.t) ~(gen_else : t Q.Generator.t) : t Q.Generator.t =
  Q.Generator.map ~f:Fir.Expression.ternary
    (Fir.Expr_ternary.quickcheck_generator_ite ~gen_if ~gen_then ~gen_else)

(** [recursive_generators ?truth ?ibop mu ~int_ctx ~bool_ctx ~k_gctx] contains
    the various constant-value expression generators that either recurse
    directly over the same generator ([mu]), or summon other expression
    generators which may or may not recurse back into this constant generator
    ([int_ctx], which has integer-typed generators; [bool_ctx], which has
    bool-typed generators; and [k_gctx], which has generators typed the same
    as the constant in question).
    
    The other parameters are as follows:

    - [truth] is a recursive version of the constant generator that 
      generates always-true values, if different from [mu]; if absent, use [mu]
      instead;
    *)
let recursive_generators
    ?(truth : t Q.Generator.t option)
    ?(falsehood : t Q.Generator.t option)
    (mu : t Q.Generator.t)
    ~(int_gctx : gctx)
    ~(bool_gctx : gctx)
    ~(k_gctx: gctx)
  ...
  kbop_generators int_gctx
  @
  kbop_generators bool_gctx
  @
  List.filter_opt
    [ Some (2.0, ternary ~gen_if:truth ~gen_then:mu ~gen_else:k_gctx.arb)
    ; Some (2.0, ternary ~gen_if:falsehood ~gen_then:k_gctx.arb ~gen_else:mu)
    ;  (* This should actually always resolve. *)
      Option.map k_gctx.ibop
        ~f:(fun bop ->
          ( 3.0
          , Q.Generator.Let_syntax.(
              let%bind x = half mu and y = half mu in
              bop (Two (x, y))) )) ]

let rec_on_other_constant (k : Fir.Constant.t) ~(other_k : Fir.Constant.t)
  (mu : Fir.Constant.t -> t Q.Generator.t)
  : t Q.Generator.t option =
  if Fir.Constant.(k = other_k)
  then None
  else Some Q.Generator.(
    (* The laziness has to cover calling [mu] here; else, there's an infinite
       loop of constructing [rec_on_other_constant]. *)
    of_lazy @@ lazy Let_syntax.(
      let%bind size = size in
...
let gen (k : Fir.Constant.t) (env : env) ~(int : env -> t Q.Generator.t)
  ...
  let int_load = Option.some_if has_ints (Q.Generator.of_lazy (lazy (int_load kv_env))) in
  let bool_load = Option.some_if has_bools (Q.Generator.of_lazy (lazy (bool_load kv_env))) in
  let rec mu (k : Fir.Constant.t) = (
    ...
    Q.Generator.weighted_recursive_union (base_generators k)
        ~f:(recursive_generators ~int_gctx ~bool_gctx ~k_gctx ?truth ?falsehood)
  )
  in mu k
