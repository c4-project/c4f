...
val make :
  src:Mini_expression.t -> dst:Mini_address.t -> mo:Mem_order.t -> t
...
val dst : t -> Mini_address.t
(** [dst st] gets [st]'s destination address. *)

val src : t -> Mini_expression.t
...
module Quickcheck_generic
    (Src : Act_utils.My_quickcheck.S_with_sexp
             with type t := Mini_expression.t)
    (Dst : Act_utils.My_quickcheck.S_with_sexp with type t := Mini_address.t) :
