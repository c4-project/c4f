...
let func_signature (id : Identifier.t) (parameters : Type.t id_assoc) :
    Ast.Declarator.t =
...
let mem_order_to_expr (mo : Mem_order.t) : Ast.Expr.t =
  Identifier (Identifier.of_string (Mem_order.to_string mo))
...
let func_body (ds : Initialiser.t id_assoc) (ss : Statement.t list) :
    Ast.Compound_stm.t =
