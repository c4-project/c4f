...
module Acc = Act_compiler

let%test_module "accessors" = (module struct
  ...
  let localhost_sims : Act_sim.Spec.Set.t =
    Or_error.ok_exn
      (Act_sim.Spec.Set.of_list
         [ Act_sim.Spec.With_id.make
             ~id:(Ac.Id.of_string "herd")
  ...
  let machines : Acc.Machine_spec.Set.t =
    Or_error.ok_exn
      (Acc.Machine_spec.Set.of_list
         [ Acc.Machine_spec.With_id.make
  ...
  let%expect_test "defaults" =
    ...
    [%expect {| ((Try Compiler (localhost gcc x86 normal)) (Try Arch (x86 att))) |}]
  ...
  let%expect_test "machines" =
    Fmt.pr "@[%a@]@." Act_compiler.Machine_spec.Set.pp (Global.machines global);
