...
let%test_module "convert_states" = (module struct
  let test (input : (string, string) List.Assoc.t list) : unit =
    let entries =
      ...
      |> Or_error.ok_exn in
    let output = Src.Dnf.convert_states (Set.of_list (module Src.Entry) entries) in
    Fmt.pr "@[%a@]@." (Act_litmus.Postcondition.pp ~pp_const:String.pp) output
