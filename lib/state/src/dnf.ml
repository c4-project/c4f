...
let predicate_of_states :
    Set.M(Entry).t -> string Act_litmus.Postcondition.Pred.t =
  Act_litmus.Postcondition.Pred.(
    Set.fold ~init:(bool true) ~f:(fun acc entry ->
...
Plumbing.Filter.Make (struct
  ...
  let run (ctx : unit Plumbing.Filter_context.t) (ic : Stdio.In_channel.t)
    ...
    Or_error.Let_syntax.(
      let%map obs =
        Observation.load_from_ic
          ~path:Plumbing.(Input.to_string (Filter_context.input ctx))
          ic
      in
