...
Fmt.(
  pr "@[%a@]@."
    (result ~error:Error.pp ~ok:(using Set.to_list (list ~sep:sp Act_common.Litmus_id.pp))))
  result
  ...
  ( module struct
    ...
    let random_state : Src.Program_actions.Label_payload.t Lazy.t =
      Lazy.Let_syntax.(
        ...
        Src.Program_actions.Label_payload.make ~name:(Act_common.C_id.of_string "label_mclabelface") ~path)
