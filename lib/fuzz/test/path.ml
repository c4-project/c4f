...
let%test_module "s-expression serialisation" =
  ( module struct
    ...
    let test (x : Program.t) : unit = print_s [%sexp (x : Program.t)]

    let%expect_test "this-statement example" =
      test
        ( Program.in_thread 0 @@ Thread.in_stms @@ Stms.in_stm 2 @@ Stm.in_if
    ...
    let%expect_test "insert-statement example" =
      test (Program.in_thread 3 @@ Thread.in_stms @@ Stms.insert 0) ;
      [%expect {| (In_thread 3 (In_stms (Insert 0))) |}]

    let%expect_test "if-statement conditional example" =
      test
        ( Program.in_thread 2 @@ Thread.in_stms @@ Stms.in_stm 9 @@ Stm.in_if
        @@ If.this_cond ) ;
