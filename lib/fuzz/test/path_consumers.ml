        ...
        let%test_module "insert_stm" =
          ( module struct
            let test_insert (path : F.Path.stm_list) : unit =
              test
                (F.Path_consumers.Statement_list.insert_stm path
          ...
          ("Can't use this statement-list path here"
           (here lib/fuzz/src/path_consumers.ml:112:65) (context insert_stm)
        ...
        let%test_module "transform_stm" =
          ( module struct
            let test_transform (path : F.Path.stm_list) : unit =
              test
                (F.Path_consumers.Statement_list.transform_stm path
          ...
          if (false) { atomic_store_explicit(y, 95, memory_order_seq_cst); } |}]
            ...
            let%expect_test "transform an insertion (invalid)" =
              ...
              [%expect
                {|
          ("Can't use this statement-list path here"
           (here lib/fuzz/src/path_consumers.ml:123:68) (context transform_stm)
           (path (Insert 2))) |}]
          end )
        ...
        let%test_module "transform_stm_list" =
          ( module struct
            ...
            let test_transform_list (path : F.Path.stm_list) : unit =
              test
                (F.Path_consumers.Statement_list.transform_stm_list path
                   ~f:iffify ~target:(Lazy.force body_stms))
            ...
            let%expect_test "list-transform an insertion (invalid)" =
              ...
              [%expect
                {|
                  ("Can't use this statement-list path here"
                   (here lib/fuzz/src/path_consumers.ml:135:16) (context transform_stm_list)
                   (path (Insert 2))) |}]
                ...
                ~f:(fun path ->
                  [%test_result: unit Or_error.t] ~here:[[%here]]
                    ...
                    (Or_error.map ~f:(Fn.const ())
                       (F.Path_consumers.Statement_list.transform_stm_list
