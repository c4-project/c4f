      ...
      ( module struct
        ...
        let%test_module "insert_stm" =
          ...
          ("Can't use this statement-list path here"
           (here lib/fuzz/src/path_consumers.ml:162:65) (context insert_stm)
           (path (On_range 1 2))) |}]
          ...
          do { atomic_store_explicit(x, 44, memory_order_seq_cst); } while (4 == 5); |}]
            ...
            let%expect_test "transform an insertion (invalid)" =
              ...
              [%expect
                {|
          ("Can't use this statement-list path here"
           (here lib/fuzz/src/path_consumers.ml:174:68) (context transform_stm)
           (path (Insert 2))) |}]
          end )
          ...
          ( module struct
            ...
            let%expect_test "list-transform an insertion (invalid)" =
              ...
              [%expect
                {|
                  ("Can't use this statement-list path here"
                   (here lib/fuzz/src/path_consumers.ml:187:16) (context transform_stm_list)
                   (path (Insert 2))) |}]
