...
let%test_module "pp" = (module struct
  let test (flag : Src.Flag.t) : unit =
    Fmt.pr "@[%a@]@." Src.Flag.pp flag
...
let%test_module "to_exact_opt" = (module struct
    let test (flag : Src.Flag.t) : unit =
...
module Flag_gen (B : sig
  ...
  let quickcheck_generator = Q.Generator.Let_syntax.(
    let%map wins = B.wins_generator
    and losses = B.losses_generator in
...
let%test_module "exact-flag recognition" = (module struct
  let%test_unit "flags with positive wins and 0 losses are always exact-true" =
    Q.Test.run_exn
      (module Gen_true_flag)
      ~f:(fun f ->
        [%test_result: bool option] ~here:[[%here]] ~equal:[%equal: bool option] ~expect:(Some true) (Src.Flag.to_exact_opt f)
      )

   let%test_unit "flags with positive losses and 0 wins are always exact-false" =
    Q.Test.run_exn
      (module Gen_false_flag)
      ~f:(fun f ->
        [%test_result: bool option] ~here:[[%here]] ~equal:[%equal: bool option] ~expect:(Some false) (Src.Flag.to_exact_opt f)
      )
end)
...
let%test_module "use as quickcheck_generators" = (module struct
  let%test_unit "always-true flags always generate true" =
    Q.Test.run_exn
      (module To_bool_generator (struct let flag = Src.Flag.exact true end))
      ~f:([%test_result: bool] ~here:[[%here]] ~equal:[%equal: bool] ~expect:true)

   let%test_unit "always-false flags always generate false" =
    Q.Test.run_exn
      (module To_bool_generator (struct let flag = Src.Flag.exact false end))
      ~f:([%test_result: bool] ~here:[[%here]] ~equal:[%equal: bool] ~expect:false)
end)
