...
module Surround : Action_types.S with type Payload.t = Payload.Surround.t = struct
  ...
  let readme () : string = Act_utils.My_string.format_for_readme
   ...
   let wrap_in_loop (cond : Act_c_mini.Expression.t) (statements : Metadata.t Act_c_mini.Statement.t list)
   : Metadata.t Act_c_mini.Statement.t =
     Act_c_mini.Statement.while_loop (Act_c_mini.Statement.While.make
      ~kind:`Do_while
      ~cond
