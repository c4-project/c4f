...
module Json (Const : sig
  ...
  let init_to_json (init : (Ac.C_id.t, Const.t) List.Assoc.t) : Yojson.Basic.t =
    `Assoc (Tx.Alist.bi_map init ~left:Ac.C_id.to_string ~right:Const.to_json)

  let postcondition_to_json (pc : Const.t Ast_base.Postcondition.t) : Yojson.Basic.t =
    `String (Fmt.strf "@[<h>%a@]" (Pp.Generic.pp_post ~pp_const:Const.pp) pc)
