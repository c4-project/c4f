...
let pp_pred : int Src.Predicate.t Fmt.t =
  Src.Predicate.pp ~pp_const:Int.pp

let eval_int : int Src.Predicate.Element.t -> bool =
  function
  | Eq (_, y) -> Int.is_non_negative y
  | Bool x -> x

let eval_pred : int Src.Predicate.t -> bool =
  Src.Postcondition_eval.eval_pred ~elt:eval_int

let eval_fail_message
    ~(expect : int Src.Predicate.t)
    (actual : int Src.Predicate.t) : string =
  Fmt.str "'%a' does not evaluate to the same result as '%a'"
    pp_pred actual
    pp_pred expect
...
let%test_module "optimising_and" = (module struct
  let opt_and_list (xs : int Src.Predicate.t list)
    : int Src.Predicate.t =
  ...
  let%test_unit "optimising_and_seq on singleton is the identity" =
    Test.run_exn
      (module struct
        type t = int Src.Predicate.t
        [@@deriving sexp, quickcheck]
      end)
      ~f:(fun x -> [%test_result: int Src.Predicate.t]
             ~here:[[%here]]
             ~expect:x
             (Src.Predicate.optimising_and_seq (Sequence.singleton x)))

  let%test_unit "optimising_and_seq on list append behaves like and of optimising_and_seqs" =
    Test.run_exn
      (module struct
...
let%test_module "optimising_or" = (module struct
  let opt_or_list (xs : int Src.Predicate.t list)
    : int Src.Predicate.t =
  ...
  let%test_unit "optimising_or_seq on list append behaves like or of optimising_or_seqs" =
    Test.run_exn
      (module struct
        type t = int Src.Predicate.t list * int Src.Predicate.t list
        [@@deriving sexp, quickcheck]
