...
\node[tra] (bas) at (0,0) {\tstack{bastardiser}};
\node[tra] (cut) at (5.5,0) {\tstack{compiler-under-test}};
\node[tra] (lit) at (11,0) {\tstack{stress-testing \\ environment}};
\node[tra] (cmp) at (5.5,3) {\tstack{comparator}};
\node[tra] (sim) at (11,3) {\tstack{assembly \\ simulator}};
\node (cmm) at (0,5) {C memory model};
\node (bug) at (5.5,5) {bug?};

\draw[arr] (tcg) to [auto] node {\tstack{plain C program}} (bas);
...
\draw[arr] (lit) to [auto, in=0, out=120] node {\tstack{outcomes actually \\ observed on machine}} ([yshift=-1mm]cmp.east);
\draw[arr] (sim) to [auto, swap] node {\tstack{outcomes allowed \\ by architecture}} (cmp);
\draw[arr] (tcg) to [auto] node {\tstack{outcomes \\ allowed in C}} (cmp);
\draw[arr] (cmp) to (bug);
\draw[arr] (cmm) to (tcg);
...
\item[\blacknum{2}]
We apply various mutations to these testcases (such as dead-code introduction~\cite{lidbury+15}) in the hope of coaxing the compiler-under-test into revealing bugs.
These mutations are designed not to interfere with the functionality of the original program, so that the postcondition should still hold. 
...
\item[\blacknum{5}]
If any outcome produced by step \blacknum{4a} or \blacknum{4b} is not allowed at the C level, we report a bug.
